    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mx-auto relative border">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                ...
                <td class="px-6 py-4 overflow-hidden" title="{{$product->description}}">{{ $product->description }}</td>
                <td class="px-6 py-4">{{ $product->amount }}</td>
                <td class="px-6 py-4 columns-3xs overflow-scroll">
                    @if ($product->url)
                        <img src="{{ asset('storage/images/'.$product->url) }}" alt="Product Image" class="flex max-w-[100px] max-h-[70px] mx-auto">
                    @endif
                </td>                
                <td class="px-6 py-4 text-center">{{ $product->images->first()->price ?? 'N/A' }}</td>
                <td class="px-6 py-4">
