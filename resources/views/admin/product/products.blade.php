...
<div class="left-[262px] relative overflow-x-auto sm:rounded-lg p-7 w-full flex flex-col table__width">
    <div class="flex justify-center">
        <div class="relative flex justify-between w-full p-1 pb-5">
            <h1 class="text-2xl font-medium text-stone-700 dark:text-white">LISTA SLIKA<h1>
            <a href="{{ route('admin.product.create') }}" class="flex items-center gap-x-2 font-medium text-green-600 dark:text-blue-500 hover:underline pr-5" type="button">
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mx-auto relative border">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                ...
                <td class="px-6 py-2 max-h-[70px] flex overflow-hidden" title="{{$product->description}}">{{ $product->description }}</td>
                <td class="px-6 py-4 text-center">{{ $product->amount }}</td>
                <td class="px-6 py-4 columns-3xs overflow-scroll">{{ $product->url }}</td>
                <td class="px-6 py-4 flex items-center justify-center">{{ $product->price }}</td>
                <td class="px-6 py-4">
