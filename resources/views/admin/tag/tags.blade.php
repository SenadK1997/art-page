    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mx-auto relative border">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $tag->name }}</td>
                <td class="px-6 py-4">
                    <a href="{{ route('admin.tag.remake', $tag->id) }}" class="flex items-center justify-end gap-x-2 font-medium text-blue-600 dark:text-blue-500 hover:underline">
                    ...
                    <form class="delete-form flex justify-end" action="{{ route('admin.tag.trash', $tag->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="flex items-center gap-x-2 delete-btn text-red-600" data-id="{{ $tag->id }}">
